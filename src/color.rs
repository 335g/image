...
pub enum ColorType {
    ...
    /// Pixel contains C, M, Y and K channels
    CMYK(u8),
}
...
pub fn bits_per_pixel(c: ColorType) -> usize {
    match c {
        ...
        ColorType::RGB(n) | ColorType::Palette(n) | ColorType::CMYK(n) => 3 * n as usize,
...
pub fn num_components(c: ColorType) -> usize {
    match c {
        ColorType::Gray(_)    => 1,
        ColorType::GrayA(_)   => 2,
        ColorType::RGB(_) | ColorType::Palette(_) | ColorType::CMYK(_) => 3,
        ColorType::RGBA(_)    => 4,
    }
}
